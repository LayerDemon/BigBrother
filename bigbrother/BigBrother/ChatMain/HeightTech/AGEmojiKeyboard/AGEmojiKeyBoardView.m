...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(id<AGEmojiKeyboardViewDataSource>)dataSource {
    ...
    if (self) {
        ...
        self.segmentsBar.frame = CGRectMake(-2,
                                            CGRectGetHeight(self.bounds)-50,
                                            CGRectGetWidth(self.bounds)/7*6,
        ...
        self.segmentsBar.tintColor = _333333;
        [self.segmentsBar setDividerImage:[[UIImage alloc]init] forLeftSegmentState:UIControlStateNormal rightSegmentState:UIControlStateNormal barMetrics:UIBarMetricsDefault];
        [self.segmentsBar setBackgroundImage:[[UIImage alloc]init] forState:UIControlStateSelected barMetrics:UIBarMetricsDefault];
        [self.segmentsBar setBackgroundImage:[UIImage imageNamed:@"bt_search_btn"] forState:UIControlStateNormal barMetrics:UIBarMetricsDefault];
        
        //发送按钮
        self.sendBtn = [UIButton buttonWithType:UIButtonTypeCustom];
//        self.sendBtn.backgroundColor = ARGB_COLOR(255, 68, 1, 1);
        [self.sendBtn setTitle:@"发送" forState:UIControlStateNormal];
        [self.sendBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [self.sendBtn setTitleColor:[[UIColor whiteColor] colorWithAlphaComponent:0.5] forState:UIControlStateHighlighted];
        [self.sendBtn setTitleColor:_DDDDDD forState:UIControlStateSelected];
        self.sendBtn.titleLabel.font = [UIFont systemFontOfSize:FLEXIBLE_NUM(14)];
        self.sendBtn.frame = CGRectMake(CGRectGetMaxX(self.segmentsBar.frame)-2,
                                   CGRectGetMinY(self.segmentsBar.frame),
                                   CGRectGetWidth(self.bounds)/7+2,
                                   CGRectGetHeight(self.segmentsBar.bounds));
        ...
        CGRect pageControlFrame = CGRectMake((CGRectGetWidth(self.bounds) - pageControlSize.width) / 2,
                                             CGRectGetHeight(self.bounds) - pageControlSize.height*2+10,
        ...
        [self.pageControl setOriginY:100];
...
//    if (sender.selectedSegmentIndex == AGEmojiKeyboardViewCategoryImageSend) {
//        self.segmentsBar.selectedSegmentIndex = self.defaultSelectedCategory;
//        return;
//    }
...
{
    [self.delegate emojiKeyBoardViewDidPressSendBtn:self];
    [self refreshSendBtnState];
}
...
{
    NSString *text = @"";
    if (self.superTextField) {
        text = self.superTextField.text;
    }
    if (self.superTextView.text) {
        text = self.superTextView.text;
    }
    if (self.superTextField.text.length > 0 || self.superTextView.text.length) {
        self.sendBtn.backgroundColor = ARGB_COLOR(255, 68, 1,0.8);
        self.sendBtn.selected = NO;
        self.sendBtn.userInteractionEnabled = YES;
    }else{
        self.sendBtn.backgroundColor = ARGB_COLOR(243, 245, 246, 1);
