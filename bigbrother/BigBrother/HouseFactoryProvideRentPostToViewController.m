//
//  HouseFactoryProvideRentPostToViewController.m
//  BigBrother
//
...
@interface HouseFactoryProvideRentPostToViewController ()<CTAssetsPickerControllerDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UIActionSheetDelegate,UIScrollViewDelegate>

@end

@implementation HouseFactoryProvideRentPostToViewController{
    ...
    //房屋地址相关
    UIView *houseAddressView;
    UITextField *houseAddressTextFiled;
...
-(void)reloadViews{
    ...
    houseAddressView = [self setUpHouseAddressViewWithOffset:viewHeightOffset];
    [contentView addSubview:houseAddressView];
    
    viewHeightOffset += HEIGHT(houseAddressView);
    viewHeightOffset += 10;
...
-(void)postButtonClick{
    ...
    NSString *addressString = houseAddressTextFiled.text;
    if (![XYTools checkString:addressString canEmpty:NO]) {
    ...
    [paramsDic setObject:addressString forKey:@"address"];
...
-(void)uploadWithParams:(NSMutableDictionary *)paramsDic{
    ...
    double uploadstart = [[NSDate date] timeIntervalSince1970];
    if (paramsDic) {
        [paramsDic setObject:HouseProductFactoryRoomType_ChuZu forKey:@"postType"];
    }
    [BBUrlConnection uploadNewHouseFactoryInfoWithParams:paramsDic complete:^(NSDictionary *resultDic, NSString *errorString) {
        double uploadend = [[NSDate date] timeIntervalSince1970];
        float delay = uploadend-uploadstart >5?0.2:5-(uploadend-uploadstart);
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [indicatorView stopAnimating:YES];
...
-(UIView *)setUpPriceViewWithOffset:(float)offset{
    ...
    unitNoteLabel.frame = (CGRect){WIDTH(view)-10-65,0,65,HEIGHT(noteLabel)};
    unitNoteLabel.text = @"元/㎡/天";
...
-(void)provideAreaViewTap{
    ...
    [PopUpSelectedView showFilterSingleChooseViewWithArray:provideAreaListArray withTitle:@"所在地区" target:self labelTapAction:@selector(provideAreaActiveSelectClick:)];
}
...
#pragma mark - 房屋地址 view 及相关事件
-(UIView *)setUpHouseAddressViewWithOffset:(float)offset{
    UIView *view = [[UIView alloc] init];
    view.frame = (CGRect){0,offset,WIDTH(contentView),titleLineHeight};
    view.backgroundColor = [UIColor whiteColor];
    ...
    houseAddressTextFiled = [[UITextField alloc] init];
    houseAddressTextFiled.frame = (CGRect){
        RIGHT(noteLabel)+10,
        0,
        WIDTH(view)-(RIGHT(noteLabel)+10)-10,
        titleLineHeight
    };
    houseAddressTextFiled.textColor = RGBColor(80, 80, 80);
    houseAddressTextFiled.font = Font(15);
    houseAddressTextFiled.minimumFontSize = 10;
    houseAddressTextFiled.adjustsFontSizeToFitWidth = YES;
    houseAddressTextFiled.returnKeyType = UIReturnKeyDone;
    houseAddressTextFiled.textAlignment = NSTextAlignmentRight;
    houseAddressTextFiled.keyboardType = UIKeyboardTypeDefault;
    [houseAddressTextFiled setAutocapitalizationType:UITextAutocapitalizationTypeNone];
    [houseAddressTextFiled setAutocorrectionType:UITextAutocorrectionTypeNo];
    
    [houseAddressTextFiled setLeftView:[[UIView alloc] initWithFrame:(CGRect){0,0,5,HEIGHT(houseAddressTextFiled)}]];
    [houseAddressTextFiled setRightView:[[UIView alloc] initWithFrame:(CGRect){0,0,5,HEIGHT(houseAddressTextFiled)}]];
    houseAddressTextFiled.leftViewMode = UITextFieldViewModeAlways;
    houseAddressTextFiled.rightViewMode = UITextFieldViewModeAlways;
    
    [view addSubview:houseAddressTextFiled];
...
-(void)resignAllInputs{
    ...
    [houseAddressTextFiled resignFirstResponder];
