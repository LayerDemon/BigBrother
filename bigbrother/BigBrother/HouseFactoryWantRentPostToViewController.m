//
//  HouseFactoryWantRentPostToViewController.m
//  BigBrother
//
...
@interface HouseFactoryWantRentPostToViewController ()<CTAssetsPickerControllerDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UIActionSheetDelegate,UIScrollViewDelegate>

@end

@implementation HouseFactoryWantRentPostToViewController{
...
-(void)uploadWithParams:(NSMutableDictionary *)paramsDic{
    ...
    double uploadstart = [[NSDate date] timeIntervalSince1970];
    if (paramsDic) {
        [paramsDic setObject:HouseProductFactoryRoomType_QiuZu forKey:@"postType"];
    }
...
-(void)provideAreaViewTap{
    ...
    [PopUpSelectedView showFilterSingleChooseViewWithArray:provideAreaListArray withTitle:@"期望地区" target:self labelTapAction:@selector(provideAreaActiveSelectClick:)];
}
