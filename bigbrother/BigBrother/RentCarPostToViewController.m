...
@interface RentCarPostToViewController ()<CTAssetsPickerControllerDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UIActionSheetDelegate,UIScrollViewDelegate>

@end

@implementation RentCarPostToViewController{
    ...
    //租车服务车型相关
    UIView *serviceCarTypeView;
    UILabel *serviceCarTypeInfoLabel;
    NSArray *serviceCarTypeListDataArray;
    NSArray *selectedCarTypeListArray;
...
-(void)postButtonClick{
    ...
    if (!selectedCarTypeListArray || selectedCarTypeListArray.count == 0) {
        [BYToastView showToastWithMessage:@"未选择服务车辆信息"];
        return;
    }
    ...
    [paramsDic setObject:selectedCarTypeListArray forKey:@"carTypeList"];
    ...
    if (isSupply) {
        ...
        NSString *dayCountString = rentDaysTextField.text;
        if (![XYTools checkString:dayCountString canEmpty:NO]) {
            [BYToastView showToastWithMessage:@"出租天数不能包含特殊符号"];
            return;
        }
        [paramsDic setObject:dayCountString forKey:@"dayCount"];
        
        NSString *personCountString = personCountTextField.text;
        if (![XYTools checkString:personCountString canEmpty:NO]) {
            [BYToastView showToastWithMessage:@"人数选择不能包含特殊符号"];
            return;
        }
        [paramsDic setObject:personCountString forKey:@"personCount"];
...
-(void)uploadWithParams:(NSMutableDictionary *)paramsDic{
    ...
    double uploadstart = [[NSDate date] timeIntervalSince1970];
    [BBUrlConnection uploadNewRentCarInfoWithParams:paramsDic complete:^(NSDictionary *resultDic, NSString *errorString) {
        double uploadend = [[NSDate date] timeIntervalSince1970];
        float delay = uploadend-uploadstart >5?0.2:5-(uploadend-uploadstart);
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [indicatorView stopAnimating:YES];
...
-(void)reloadViews{
    if (isSupply) {
        ...
        [rentDaysView removeFromSuperview];
        //        if (rentDaysView) {
        //            CGRect newRect = rentDaysView.frame;
        //            newRect.origin.y = viewHeightOffset;
        //            rentDaysView.frame = newRect;
        //        }else{
        //            rentDaysView = [self setUpRentDaysViewWithOffset:viewHeightOffset];
        //        }
        //        [contentView addSubview:rentDaysView];
        //        
        //        viewHeightOffset += HEIGHT(rentDaysView);
        //        
        //        viewHeightOffset += 10;
        ...
        [serviceCarTypeView removeFromSuperview];
        if (serviceCarTypeView) {
            CGRect newRect = serviceCarTypeView.frame;
            newRect.origin.y = viewHeightOffset;
            serviceCarTypeView.frame = newRect;
        }else{
            serviceCarTypeView = [self setUpServiceCarTypeViewWithOffset:viewHeightOffset];
        }
        [contentView addSubview:serviceCarTypeView];
        
        viewHeightOffset += HEIGHT(serviceCarTypeView);
        ...
        [personCountView removeFromSuperview];
        //        if (personCountView) {
        //            CGRect newRect = personCountView.frame;
        //            newRect.origin.y = viewHeightOffset;
        //            personCountView.frame = newRect;
        //        }else{
    ...
    selectedCarTypeListArray = @[];
    serviceCarTypeInfoLabel.text = @"";
...
-(UIView *)setUpProvideOrNotViewWithOffset:(float)offset{
    ...
    [provideButton setTitle:@"  出租车辆" forState:UIControlStateNormal];
    ...
    [needButton setTitle:@"  求租车辆" forState:UIControlStateNormal];
...
#pragma mark - 租用天数 view 及相关事件
-(UIView *)setUpRentDaysViewWithOffset:(float)offset{
    UIView *view = [[UIView alloc] init];
    view.frame = (CGRect){0,offset,WIDTH(contentView),titleLineHeight};
    view.backgroundColor = [UIColor whiteColor];
    ...
    rentDaysTextField = [[UITextField alloc] init];
    rentDaysTextField.frame = (CGRect){
        RIGHT(noteLabel)+10,
        0,
        WIDTH(view)-(RIGHT(noteLabel)+10)-10,
        titleLineHeight
    };
    rentDaysTextField.textColor = RGBColor(80, 80, 80);
    rentDaysTextField.font = Font(15);
    rentDaysTextField.minimumFontSize = 10;
    rentDaysTextField.textAlignment = NSTextAlignmentRight;
    rentDaysTextField.adjustsFontSizeToFitWidth = YES;
    rentDaysTextField.returnKeyType = UIReturnKeyDone;
    rentDaysTextField.keyboardType = UIKeyboardTypeNumberPad;
    [rentDaysTextField setAutocapitalizationType:UITextAutocapitalizationTypeNone];
    [rentDaysTextField setAutocorrectionType:UITextAutocorrectionTypeNo];
    
    [rentDaysTextField setLeftView:[[UIView alloc] initWithFrame:(CGRect){0,0,5,HEIGHT(rentDaysTextField)}]];
    [rentDaysTextField setRightView:[[UIView alloc] initWithFrame:(CGRect){0,0,5,HEIGHT(rentDaysTextField)}]];
    rentDaysTextField.leftViewMode = UITextFieldViewModeAlways;
    rentDaysTextField.rightViewMode = UITextFieldViewModeAlways;
    
    [view addSubview:rentDaysTextField];
...
#pragma mark - 服务车型 view 及相关事件
-(UIView *)setUpServiceCarTypeViewWithOffset:(float)offset{
    UIView *view = [[UIView alloc] init];
    view.frame = (CGRect){0,offset,WIDTH(contentView),titleLineHeight};
    view.backgroundColor = [UIColor whiteColor];
    
    UITapGestureRecognizer *serviceCarTypeTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(serviceCarTypeTap)];
    view.userInteractionEnabled = YES;
    ...
    serviceCarTypeInfoLabel = [[UILabel alloc] init];
    serviceCarTypeInfoLabel.frame = (CGRect){RIGHT(noteLabel)+10,0,WIDTH(view)-RIGHT(noteLabel)-10-10,HEIGHT(view)};
    serviceCarTypeInfoLabel.text = @"";
    serviceCarTypeInfoLabel.textColor = RGBColor(50, 50, 50);
    serviceCarTypeInfoLabel.textAlignment = NSTextAlignmentRight;
    serviceCarTypeInfoLabel.font = Font(14);
    [view addSubview:serviceCarTypeInfoLabel];
    
    [self getRentCarServiceCarTypeList];
    return view;
}

-(void)serviceCarTypeTap{
    [self resignAllInputs];
    if (!serviceCarTypeListDataArray || serviceCarTypeListDataArray.count == 0) {
        [BYToastView showToastWithMessage:@"数据不存在,请稍候再试"];
        [self getRentCarServiceCarTypeList];
        return;
    }
    
    [PopUpSelectedView showFilterMutitypeChooseViewWithArray:serviceCarTypeListDataArray withTitle:@"服务车型" normalTextColor:RGBColor(100, 100, 100) selectedTextColor:BB_BlueColor doneButtonBlock:^(NSArray *selectedIndexArray) {
...
-(void)serviceCarTypeSelectFinish:(NSArray *)array{
    ...
    NSMutableArray *selectedCarTypeListArrayTmp = [NSMutableArray array];
    
    for (int i = 0; i < array.count; i++) {
        BOOL select = [array[i] boolValue];
        if (select) {
            NSDictionary *dic = serviceCarTypeListDataArray[i];
            NSString *name = dic[@"name"];
            if (!name) {
                name = @"";
            }
            ...
            [selectedCarTypeListArrayTmp addObject:@{@"id":@(cartypeID),@"name":name,@"orderBy":@(orderBy)}];
            orderBy += 1;
        }
    }
    selectedCarTypeListArray = [NSArray arrayWithArray:selectedCarTypeListArrayTmp];
    
    NSString *labelShowString = [labelShowedStringArray componentsJoinedByString:@"、 "];
    serviceCarTypeInfoLabel.text = labelShowString;
}

//-(void)serviceCarTypeSelectClick:(UITapGestureRecognizer *)tap{
//    if (!serviceCarTypeListDataArray || serviceCarTypeListDataArray.count == 0) {
//        return;
//    }
//    NSDictionary *dic = serviceCarTypeListDataArray[tap.view.tag];
//    serviceCarTypeInfoDic = dic;
//    NSString *name = dic[@"name"];
//    serviceCarTypeInfoLabel.text = name;
//    [PopUpSelectedView dismissFilterChooseView];
//}

-(void)getRentCarServiceCarTypeList{
    [BBUrlConnection getAllRentCarTypeListComplete:^(NSDictionary *resultDic, NSString *errorString) {
        ...
        serviceCarTypeListDataArray = [NSArray arrayWithArray:gategroyTmpShowedArray];
    }];
}
...
-(void)resignAllInputs{
    ...
    [rentDaysTextField resignFirstResponder];
